        ...
        for log_path in [LOG_FILE, BAT_LOG_FILE]:
            ...
            if idade_dias >= LOG_RETENTION_DAYS:
                ...
                with open(log_path, "w", encoding="utf-8") as f:
                    f.write(
                        f"[{agora.strftime('%Y-%m-%d %H:%M:%S')}] Log rotacionado (idade: {idade_dias} dias)\n"
                    )

                log_debug(
                    f"Log rotacionado: {os.path.basename(log_path)} (idade: {idade_dias} dias)"
                )
...
def log(level: str, message: str, details: str = ""):
    """Registra mensagens de log genéricas"""
    try:
        with open(LOG_FILE, "a", encoding="utf-8") as f:
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def load_db():
    try:
        ...
        with open(DB_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
            log_debug(
                f"DB carregado: {len(data.get('users', []))} usuários, {len(data.get('passwords', []))} senhas"
            )
            return data
    except Exception as e:
...
def read_message():
    ...
    try:
        ...
        if len(message_bytes) != message_length:
            log_debug(
                f"ERRO: Esperava {message_length} bytes, recebeu {len(message_bytes)}"
            )
            return None
    ...
    for candidate in candidates:
        for t in db["tokens"]:
            if t["token"] == candidate:
                if t.get("validade") is None:
                    return t
                if "validade" in t:
                    try:
                        val = datetime.datetime.strptime(
                            t["validade"], "%Y-%m-%d %H:%M:%S"
                        )
...
def process_message(msg):
    ...
    if action == "log":
        ...
        if level == "error":
            ...
            log_debug(f"[JS] {message} | {details}")
    ...
    if action == "logout":
        if authorization:
            ...
            db["tokens"] = [
                t
                for t in db["tokens"]
                if t["token"] not in [authorization, token_str_to_remove]
            ]
    ...
    if action == "tokensinvalidos":
        ...
        db["tokens"] = [
            ...
            for t in db["tokens"]
            if t.get("validade") is None
            or datetime.datetime.strptime(t["validade"], "%Y-%m-%d %H:%M:%S") > now
        ]
    ...
    if action in ["update_user", "update_user_part"]:
        ...
        log_debug(
            f"Campos recebidos - id: {uid}, nome: {nome}, login: {login_new}, senha: {'***' if senha_new else None}"
        )
    ...
    if action == "excluir_senha":
        ...
        # Remove
        db["passwords"] = [
            p for p in db["passwords"] if int(p["id_senha"]) != int(id_senha)
        ]
        save_db(db)
